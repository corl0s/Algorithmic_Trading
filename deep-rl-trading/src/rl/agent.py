            ...
            try:
                for ep in range(self.n_eps):
                    ...
                    while not done:
                        ...
                        # print(f"Reward type: {type(reward)}, reward: {reward}")# Convert one-element numpy array reward to scalar float
                    ...
                    reward = float(np.round(np.sum(_rewards), 4))
                    rewards.append(reward)
                    ...
                    cum_rewards.append(cumulative_reward)
                    avg_reward = float(np.round(np.mean(rewards[-25:]), 4))
                    avg_cum_reward = float(np.round(np.mean(cum_rewards[-25:]), 4))
                    lr = float(self.target_net.optimizer.param_groups[0]['lr'])
                    
                    # Convert loss tensor to Python float scalar safely
                    loss = loss.detach().item()
                    loss = round(loss, 4)
                    losses.append(loss)
                    
                    avg_loss = float(np.round(np.mean(losses[-25:]), 4))
    ...
    def test(self):
        ...
        for file in os.listdir(p):
            ...
            if ".DS" not in file:
                ...
                self.env.render(action_types, wait=True)
        return rewards
...
class ReplayMemory:
    ...
    def store(self, states, actions, rewards, next_states):
        ...
        elif isinstance(rewards, list) and len(rewards) == 1:
